...
public function getEnterpriseWarehouses()
{
    // Fetch warehouses directly with their users and subscriptions
    $warehouses = Warehouse::whereHas('user.subscriptions', function ($query) {
        $query->where('package_id', 3); // Filter by package_id = 3 (enterprise)
    })->with('user')->get();

    // Format the response
    $formattedWarehouses = $warehouses->map(function ($warehouse) {
        return [
            'id' => $warehouse->id, // Include warehouse ID
            'user_name' => $warehouse->user->name,
            'user_email' => $warehouse->user->email,
            'warehouse_name' => $warehouse->name,
            'warehouse_location' => $warehouse->location,
            'width' => $warehouse->width, // Include width
            'height' => $warehouse->height, // Include height
            'price_per_meter' => $warehouse->price_per_meter, // Include price per meter
            'status' => $warehouse->status, // Include price per meter
        ];
    });

    return response()->json($formattedWarehouses);
}
